...
function [PWKp,PWKn]=CV_PWM_matrices_generatures3D(pos_pattern,neg_pattern)
...
for k=1:Nb_Layers
    ...
    featuresK_pos=pos_pattern(:,:,k); 
    featuresK_neg=neg_pattern(:,:,k);
...
%% Creat the PWM 
One_vec=ones([1 Mp]);
PWRp=One_vec*double(DP);
PWRn=One_vec*double(DN);
